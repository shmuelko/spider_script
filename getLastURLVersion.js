...
page.onLoadFinished = function(status) {
	...
	page.includeJs("https://code.jquery.com/jquery-3.1.1.min.js", function() {
		globalData = page.evaluate(function() {
			...
			if($('.more:last').length == 0){
				innerData = '(' + window.location.href + ', NoPages)';
			}else{
				innerData = '(' + $('.more:last').attr('href') + ', '+ $('.more:last').parent().text().substring($('.more:last').parent().text().lastIndexOf('of')+3, $('.more:last').parent().text().lastIndexOf('|')-1) + ')';				
			}
